...
const Gagarin = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setError(error?.error?.message || 'Failed to load Gagarin data.');
            } finally {
                setLoading(false)
            }
        };
    ...
    return (
        <>
                ...
                <div className="mt-6 w-full">
                  <div className="shadow-lg p-2 rounded">
                    ...
                    <div className="px-4 py-6 grid grid-cols-2 sm:px-0">
                        ...
                        <p className="text-sm">{gagarinData?.data?.[0]?.mission?.launch_details?.launch_date}</p>
                    </div>
                    <div className="px-4 py-6 grid grid-cols-2 sm:px-0">
                        <p className="font-bold text-gray-900">Название космодрова</p>
                        <p className="text-sm">{gagarinData?.data?.[0]?.mission?.launch_details?.launch_site?.name}</p>
                    </div>
                    <div className="px-4 py-6 grid grid-cols-2 sm:px-0">
                        <p className="font-bold text-gray-900">Широта</p>
                        <p className="text-sm">{gagarinData?.data?.[0]?.mission?.launch_details?.launch_site?.location?.latitude}</p>
                    </div>
                    <div className="px-4 py-6 grid grid-cols-2 sm:px-0">
                        <p className="font-bold text-gray-900">Долгота</p>
                        <p className="text-sm">{gagarinData?.data?.[0]?.mission?.launch_details?.launch_site?.location?.longitude}</p>
                    </div>
                    <div className="px-4 py-6 grid grid-cols-2 sm:px-0">
                        <p className="font-bold text-gray-900">Продолжителность полета</p>
                        <p className="text-sm">{gagarinData?.data?.[0]?.mission?.flight_duration?.hours} час {gagarinData?.data?.[0]?.mission?.flight_duration?.minutes} минут</p>
                    </div>
                  </div>
                  <div className="mt-4 shadow-lg p-2 rounded">
                    ...
                    <div className="px-4 py-6 grid grid-cols-2 sm:px-0">
                        ...
                        <p className="text-sm">{gagarinData?.data?.[0]?.mission?.spacecraft?.manufacturer}</p>
                    </div>

                    <div className="px-4 py-6 grid grid-cols-2 sm:px-0">
                        <p className="font-bold text-gray-900">Вместимость экипажа</p>
                        <p className="text-sm">{gagarinData?.data?.[0]?.mission?.spacecraft?.crew_capacity}</p>
                    </div>
                  </div>
                

                  <div className="mt-4 shadow-lg p-2 rounded">
                    <h2 className="mt-2 font-bold text-xl"></h2>
                    <div className="px-4 py-6 grid grid-cols-2 sm:px-0">
                    ...
                    <div className="px-4 py-6 grid grid-cols-2 sm:px-0">
                        <p className="font-bold text-gray-900">Широта</p>
                        <p className="text-sm">{gagarinData?.data?.[0]?.landing?.site?.coordinates?.latitude}</p>
                    </div>
                    <div className="px-4 py-6 grid grid-cols-2 sm:px-0">
                        <p className="font-bold text-gray-900">Долгота</p>
                        <p className="text-sm">{gagarinData?.data?.[0]?.landing?.site?.coordinates?.longitude}</p>
                    </div>
                    <div className="px-4 py-6 grid grid-cols-2 sm:px-0">
          ...
          <Link className="items-center flex justify-center bg-sky-500 text-white py-2 px-2 rounded shadow-md text-xs hover:bg-sky-600" to="/missions">К списку миссий</Link>
        </div>
